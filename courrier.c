...
void case_courrier(courrier liste[], joueur *j){
  ...
  if(est_carte_assurance(liste[num]) == 1){
	...
	j->nb_courrier++;
	strcpy(j->sesCourriers[j->nb_courrier].message, liste[num].message);
	j->sesCourriers[j->nb_courrier].somme = liste[num].somme;
	j->sesCourriers[j->nb_courrier].valeur = liste[num].valeur;
	j->sesCourriers[j->nb_courrier].numero = liste[num].numero;
      }
    }
    else{
...
int a_besoin_argent(joueur j){
  int i;
  for(i = 0 ;  i < j.nb_courrier ; i++){
    if(est_besoin_argent(j.sesCourriers[i]) == 1)
      return 1;
  }
  return 0;
}
...
int main(){
  ...
  printf("%d %d %d\n", j.total, a_besoin_argent(j), j.nb_courrier);
  for(i = 0 ;  i < j.nb_courrier + 1 ; i++)
    afficher_courrier(j.sesCourriers[i]);
  exit(0);
}
